...
export async function saveToPantry(formData) {
  try {
    ...
    for (const ingredient of ingredients) {
      const response = await fetch(`${strapiUrl}/api/pantry-items`, {
        method: "POST",
        headers: {
...
export async function getPantryItems() {
  try {
    ...
    const response = await fetch(
      `${strapiUrl}/api/pantry-items?filters[owner][id][$eq]=${user.id}&sort=createdAt:desc`,
      {
        headers: {
          Authorization: `Bearer ${strapiApiToken}`,
        },
...
export async function deletePantryItem(formData) {
  try {
    ...
    const response = await fetch(`${strapiUrl}/api/pantry-items/${itemId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${strapiApiToken}`,
      },
