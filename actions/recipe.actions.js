...
export async function getOrGenerateRecipe(formData) {
  try {
    ...
    const searchResponse = await fetch(
      `${strapiUrl}/api/recipes?filters[title][$eqi]=${encodeURIComponent(
        normalizedTitle,
      )}&populate=*`,
      {
        headers: {
    ...
    if (searchResponse.ok) {
      ...
      if (searchData.data && searchData.data.length > 0) {
        ...
        const savedRecipeResponse = await fetch(
          `${strapiUrl}/api/saved-recipes?filters[user][id][$eq]=${user.id}&filters[recipe][id][$eq]=${searchData.data[0].id}`,
          {
            headers: {
              Authorization: `Bearer ${strapiApiToken}`,
            },
...
`;
    ...
    const createRecipeResponse = await fetch(`${strapiUrl}/api/recipes`, {
      method: "POST",
      headers: {
...
export async function saveRecipeToCollection(formData) {
  try {
    ...
    const existingResponse = await fetch(
      `${strapiUrl}/api/saved-recipes?filters[user][id][$eq]=${user.id}&filters[recipe][id][$eq]=${recipeId}`,
      {
        headers: {
          Authorization: `Bearer ${strapiApiToken}`,
        },
    ...
    const saveResponse = await fetch(`${strapiUrl}/api/saved-recipes`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${strapiApiToken}`,
...
export async function getRecipesByPantryIngredients() {
  try {
    ...
    const pantryResponse = await fetch(
      `${strapiUrl}/api/pantry-items?filters[owner][id][$eq]=${user.id}`,
      {
        headers: {
          Authorization: `Bearer ${strapiApiToken}`,
        },
...
export async function getSavedRecipes() {
  try {
    ...
    const response = await fetch(
      `${strapiUrl}/api/saved-recipes?filters[user][id][$eq]=${user.id}&populate[recipe][populate]=*&sort=savedAt:desc`,
      {
        headers: {
          Authorization: `Bearer ${strapiApiToken}`,
        },
